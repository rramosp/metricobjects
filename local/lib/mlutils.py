...
def plot_2D_boundary(predict, mins, maxs, n=200, 
                     line_width=3, line_color="black", line_alpha=1, label=None,
                     ax = None):
    ax = plt if ax is None else ax
    ...
    ax.contour(gd0,gd1,p, levels=levels, alpha=line_alpha, colors=line_color, linewidths=line_width)
    if label is not None:
        ax.plot([0,0],[0,0], lw=line_width, color=line_color, label=label)
    return np.sum(p==0)*1./n**2, np.sum(p==1)*1./n**2

def plot_2Ddata_with_boundary(predict, X, y, line_width=3, line_alpha=1, line_color="black", 
                              dots_alpha=.5, label=None, noticks=False, ax=None, title=None):
    
    ax = plt if ax is None else ax
    mins,maxs = np.min(X,axis=0), np.max(X,axis=0)    
    plot_2Ddata(X,y,dots_alpha, ax=ax)
    p0, p1 = plot_2D_boundary(predict, mins, maxs, line_width=line_width, 
                line_color=line_color, line_alpha=line_alpha, label=label, ax=ax )
    if noticks:
        plt.xticks([])
        plt.yticks([])
        
    if title is not None:
        if ax.__class__.__name__ == 'module':
            ax.title(title)
        else:
            ax.set_title(title)
        return p0, p1
